...
macos_prepare () {
  ...
  [[ "${downloadVer}" ]] && (($(ver "${downloadVer}") < $(ver "1.1.59.710"))) && { echo -e "${red}Error:${clear} ${downloadVer} not supported by AIMODS-Bash\n" >&2; exit 1; }
  macos_set_path
...
run_interactive_check () {
  if [[ "${interactiveMode}" ]]; then
    ...
    [[ "${platformType}" == "macOS" && -z "${installMac+x}" ]] && { { nstallMac='true'; }; }
    [[ "${platformType}" == "macOS" ]] && { blockUpdates='true'; }
    excludeExp='true'
    (($(ver "${clientVer}") >= $(ver "1.1.93.896") && $(ver "${clientVer}") <= $(ver "1.2.13.661"))) && { oldUi='false'; }
